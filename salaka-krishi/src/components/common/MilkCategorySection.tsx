...
export default function MilkCategorySection() {
    const { data, isLoading, isError } = useQuery<DataWrapper<ProductSchema[]>>({
        queryKey: ["dairy-products-filtered"],
        queryFn: () => getQueryFilterProducts({ categories: ["milk", "dairy", "khuwa"] }).then(res => res.data),
    });

    const allDairyProducts = Array.isArray(data?.data) ? data.data : (data?.data as any)?.products || [];

    // Helper to filter and sort
    const getFilteredProducts = (filter: string) => {
        let products = [...allDairyProducts];
        if (filter === "top_rated") {
    ...
    const allEmpty = !isLoading && allDairyProducts.length === 0;
