...
import { deleteMainBanner } from 'api/main-banner';

interface ProductCardProps {
    banner: MainBannerInterface;
}
export default function BannerCard({ banner }: ProductCardProps) {
    const baseUrl = process.env.NEXT_PUBLIC_BACKEND_URL;
    const bannerImage = `${baseUrl}/uploads/${banner?.imageUrl}`;
    const deleteMutation = useMutation({
        mutationFn: async (id: string) => {
            const rest = await deleteMainBanner(id);
            return rest.data;
        }
    });
    ...
    return (
        <MainCard
            ...
            <Stack
                ...
                {/* {(banner.discountPercentage ?? 0) > 0 && <Chip variant="filled" color="error" size="small" label={`${banner.discountPercentage}% Off`} />} */}
                <Chip variant="filled" color="primary" size="small" label={
