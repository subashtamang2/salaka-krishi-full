"use client";
import { Autocomplete, Button, FormControlLabel, FormHelperText, Grid, InputLabel, Stack, TextField } from "@mui/material";
...
import { ProductAvailability, ProductSchema, ProductStatus, UpdateProductSchema } from "schema/product";
...
export default function Page() {
    ...
    const productMutation = useMutation({
        mutationFn: async (payload: UpdateProductSchema) => {
            const res = await updateProduct(paramsId, payload);
            return res.data;
        },
    });

    const imageBaseUrl = process.env.NEXT_PUBLIC_IMAGE_BASE_URL || "";
    const { data: prodRes, isError, isLoading } = useQuery<DataWrapper<ProductSchema>>({
        queryKey: ['edit-product', paramsId],
        enabled: !!paramsId,
        queryFn: async () => {
            const res = await getSingleProduct(paramsId);
            return res.data;
        },
    });
    const product = prodRes?.data;

    const initialFiles: UploadFile[] | [] = product?.imageUrls
        ? product.imageUrls.map((imageUrl) =>
            Object.assign({ preview: `${imageBaseUrl}/${imageUrl}`, type: 'image' })
        )
        : [];
    const formik = useFormik({
        ...
        initialValues: {
            ...
            estimatedDeliveryMaxDays: product?.estimatedDeliveryMaxDays || 3,
        ...
        onSubmit: async (values, { setSubmitting, resetForm }) => {
            ...
            processImages(files ?? []).then((uploadedImages) => {
                const payload = {
                    ...rest,
                    price: Number(price),
                    stock: Number(stock),
    ...
    if (categoryError) {
        return <Error404 title="Categories not found" />
    }
    ...
    return (
        <>
            <Grid container spacing={1}>
                ...
                <Grid item lg={10}>
                    <MainCard title="Update Product" sx={{ mb: 3 }}>
