...
import { CurrentUser, UserInterface, UserInterfaceForm, UserRole } from "schema/schema";
...
const validationSchema = yup.object({
    ...
    password: yup.string().nullable().min(6, "Password should be at least 6 characters").optional(),
...
export default function page() {
    ...
    const initialFile: UploadFile | null = user?.profileUrl
        ? Object.assign({
            preview: `${imageBaseUrl}/${user.profileUrl}`,
        })
        : null;
    ...
    const formik = useFormik<UserInterfaceForm>({
        ...
        onSubmit: (values, { setSubmitting, resetForm }) => {
            setSubmitting(true);
            const { file, password, ...restValues } = values;
            ...
            processImages(file ?? null).then((finalValues) => {
                ...
                const data = {
                    ...restValues,
                    profileUrl: imageUrl || restValues.profileUrl,
                    ...(password?.trim() ? { password } : {}),
                };

                updateUserMutation.mutate(data, {
                    onSuccess: (data) => {
                        const { email, firstName, id, lastName, profileUrl, role } = data?.data;
    ...
    return (
        <>
            <Grid container spacing={1}>
                ...
                <Grid item lg={7}>
                    <MainCard title="Your Information" sx={{ mb: 3 }}>
                        <form onSubmit={formik.handleSubmit} id="update-your-info-form">
